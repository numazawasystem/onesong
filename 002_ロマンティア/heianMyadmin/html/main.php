		...
		if (is_uploaded_file($_FILES["upfile"]["tmp_name"])) {
			...
			if(sql10($_FILES["upfile"]["name"] , $_GET["akn"]) == 0){
				...
				if (move_uploaded_file($_FILES["upfile"]["tmp_name"], "../data/" . $filename)) {
					...
					$header .= "From: owakare-ai002@chunichi-eizo.jp\r\n";
					$header .= "Return-Path: owakare-ai002@chunichi-eizo.jp\r\n";

					// -f をつけてReturn-Pathを明示
					mb_send_mail($to, $title, $message, $header, "-fowakare-ai002@chunichi-eizo.jp");
				}
			}
		}
	}
...
?>
					<section>
						...
						<?php
							if($acs == 1){
						?>
						<p><a href="../html/download.php?akn=<?php print($_GET['akn']); ?>" class="btn-square2">画像のダウンロード</a></p>
						<?php
							}
						?>
						<?php
						...
						<?php
							if($acs == 1){
						?>
						<p><a href="../html/download2.php?akn=<?php print($_GET['akn']); ?>" class="btn-square2">画像のダウンロード</a></p>
						<?php
							}
						?>
						<?php
						...
						if($cnt > 0){
						?>
							<p><a href="../html/mpgdownload.php?akn=<?php print($_GET['akn']); ?>" class="btn-square2">音楽のダウンロード</a></p>
						<?php
						}else{
						?>
							<p><a href="" class="btn-square2" onclick="return confirm('音楽はまだアップロードされていません');">音楽のダウンロード</a></p>
						<?php
						}
						?>
						<?php
						...
						<FORM method="POST"  class="search_container2" enctype="multipart/form-data">
							...
							<div class="list3"><input type="submit" name="btnExecUP2" value="音楽のアップロード"></div>
						</form>
						<br /><br /><br />
